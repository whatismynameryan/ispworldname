...
def reconcile_countries_by_name(plot_countries, gdp_countries):
                ...
                if value==the_out_of_value:
                    for key in plot_countries:
                        if plot_countries[key]==the_out_of_value:
                            list1.append(key)    #在绘图库中和世行中都有国名，但是在世行中没有对应的GDP数据，判定为无GDP数据的绘图库代码
        else:
    ...
    missing_from_world_bank=set(list4) #包含在世行中无对应国家名的绘图库国家代码及绘图库和世行都有对应的国名但是在世行中无GDP数据的国家代码
    return (plotcountriesGDP,missing_from_world_bank)
...
render_world_map(world_bank_data,pygal_countries,year,'isp_gdp_world_name_1997.svg')
